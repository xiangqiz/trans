...
<template>
    <ul class="ul-tree"
        :class="{'first-level':!toNext,dragPadding:!toNext&&defaultTreeT.isDrag,cursorMove:defaultTreeT.isDrag && trueTreeDragData.isDraggable}"
        ...
        <li v-for="model in treeData" class="tree-wrapper">
            <div class="show-node" :class="{
                ...
                <div class="toggleBlock" @click.stop="toggle(model)">
					<span class="toggle"
                          :class="{showNextLevelTree:(trueOpenName.length>0 && trueOpenName.includes(model[defaultTreeT.id]))}">
					</span>
                </div>
                <div class="imgFolder"
                     :class="{imgFolderOpen:(trueOpenName.length>0 && trueOpenName.includes(model[defaultTreeT.id]))}"></div>
                <div class="node"
                     v-show="!(trueIsDbClicked && (trueDbSelected.length>0) && (trueDbSelected[0]==model[defaultTreeT.id]))"
            ...
            <div class="add-children"
                 v-show="isAdd===true && (trueSelected.length>0) && (trueSelected[0]==model[defaultTreeT.id])">
            ...
            <!-- 第二级往后的 -->
            <item v-if="model.children.length>0"
                  v-show="trueOpenName.length>0 && trueOpenName.includes(model[defaultTreeT.id])"
				...
				dragSelected:trueDragSelected,dragData:trueTreeDragData,isDbClicked:trueIsDbClicked}"
            ...
            <!-- getFromNextData方法，负责向外传值，但是不传到使用此插件的地方 -->
        </li>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            trueIsDbClicked () {
                if (this.toNext) {
                    return this.toNext.isDbClicked;
                } else {
                    return this.isDbClicked;
                }
            }
        },
        watch: {
        ...
        methods: {
            ...
            dragMouseDown (model, e) {
                if (!this.trueIsDbClicked && !this.isAdd && this.treeT.isDrag && !this.trueTreeDragData.isDraggable) {
                    let _this = this;
            ...
            getFromNextData (msg) {
                if (this.toNext) {
                    ...
                    if (msg.dbClicked !== undefined) {
                        this.isDbClicked = msg.dbClicked;
                    }
                }
            },
            // 鼠标离开时清楚拖拽状态
            ...
            handleMouseUp (e) {
                let parentId = this.findMouseUpNode(e);
                if (parentId === 'root') {
                    this.$emit('getDragData', {
                        dragData: this.trueTreeDragData.currentDragModel,
                        parentData: parentId
                    });
                } else {
                    // 不能移到自身文件夹内
            ...
            findShowNodes () {
                ...
                for (let i = 0; i < showNodes.length; i++) {
                    let _domtop = $(showNodes[i]).offset().top + $(showNodes[i]).outerHeight() - ulOffset.top;
                    if (_domtop >= 0) {
                        showNodesArr.push({
                            _dom: showNodes[i],
                            _pos: _domtop
            ...
            changeFolderName (model) {
                ...
                if (this.treeT.isDbClick) {
                    ...
                    if (this.toNext) {
                        this.$emit('getFromNextData', {
                            dbChoosed: model[this.defaultTreeT.id],
                            dbClicked: this.isDbClicked
                        });
                    } else {
                        this.dbSelected = [];
            ...
            // 点击小三角展开或收起树
            toggle (model) {
                if (!this.isAdd && !this.trueIsDbClicked) {
                    clearTimeout(this.dragSetTimeOut);
                    if (this.toNext) {
            ...
            chooseTreeItem (model) {
                if (!this.isAdd && !this.trueIsDbClicked) {
                    ...
                    clearTimeout(this.dragSetTimeOut);
                    clearTimeout(this.singleChooseClick);
                    clearTimeout(this.dblSettimeout);
            ...
            changeDbNewName (model) {
                ...
                setTimeout(function () {
                    let _obj = {};
                    _obj.oldModel = model;
                    _obj.newChangeName = _this.dbChangeNewName;
                    _this.$emit('changeDbNewName', _obj);
                    _this.isDbClicked = false;
                    if (_this.toNext) {
                        _this.$emit('getFromNextData', { dbClicked: _this.isDbClicked });
                    }
                    _this.dbChangeNewName = '';
                }, 300);
            },
            // 内层触发事件
            ...
            // 失去焦点改名字 setTimeout解决blur与click冲突
            changeNewName (model) {
                let _this = this;
                clearTimeout(this.newSettime);
                setTimeout(function () {
                    let _obj = {};
                    if (!_this.toNext && _this.isAdd === 'root') {
                        _obj.parent = 'root';
                    } else {
                        _obj.parent = model;
                    }
                    _obj.addValue = _this.newName;
                    _this.$emit('changeNewName', _obj);
                }, 300);
            },
            // 触发父节点事件
            sendData (model) {
...
<style lang="scss" scoped>
    $node-hover-color: #e4eceb;
    $choosed-color: #b3c7e4;
    ...
    * {
    ...
    //此处的padding-bottom是留给modelList处的拖拽功能用的。所以下拉框引用树的时候就不要此padding了
    .first-level.dragPadding {
    ...
    .node, .db-change-input, .add-root-input, .add-children-input {
        border: 1px solid #dacaca;
        @extend .inlineBlock;
        @extend .ellipsis;
        width: calc(100% - 60px);
        height: 100%;
        line-height: $showNodeHeight;
        font-size: 13px;
        color: #6c6969;
    }
